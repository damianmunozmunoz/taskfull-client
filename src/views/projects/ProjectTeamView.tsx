    ...
    if (data) return (
        <>
            ...
            <nav className="my-5 flex gap-3">
                <button
                    type="button"
                    className="bg-teal-400 hover:bg-teal-500 px-10 py-3 text-white text-xl
                        font-bold cursor-pointer transition-colors"
                    onClick={() => navigate('?addMember=true')}
                >Agregar colaborador</button>
                <Link
                    to={`/projects/${projectID}`}
                    className="bg-emerald-400 hover:bg-emerald-500 px-10 py-3 text-white text-xl
                        font-bold cursor-pointer transition-colors"
                >
                    Volver al proyecto
                </Link>
                                        ...
                                        <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                                            <Menu.Item as="div">
                                                {({ active }) => (
                                                    <button
                                                        type="button"
                                                        className={`block px-3 py-1 text-sm leading-6 ${active ? "bg-gray-100" : ""} text-red-500`}
